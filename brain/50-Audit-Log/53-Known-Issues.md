...
- **[OPEN, 1.0] Limiter regression not yet committed.** Verified offline via scratch `limiter_check` (4 cases, ceiling held). Add a JUCE-free `Tests/limiter_runner.cpp` + ctest gate (ceiling-held on an inter-sample signal, GR>0, latency) like the detail regression, so it can't regress.
